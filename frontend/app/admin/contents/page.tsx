...
import { auth } from "@/lib/firebase"; // ← 自前のFirebase初期化ファイル
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/contents/${editContent.id}`,
        {
          method: "PUT",
          headers: {
      ...
      <table className="table-auto w-full border-collapse border border-gray-300 mb-8">
        ...
        <tbody>
          {contents.map((content) => (
            <tr
              key={content.id}
              className={editContent?.id === content.id ? "bg-yellow-50" : ""}
            >
